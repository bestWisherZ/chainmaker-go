...
func New(cfg *consensusUtils.ConsensusImplConfig) (protocol.ConsensusEngine, error) {
	...
	m.msgBus.Register(msgbus.ProposeBlock, m)
...
func (m *Maxbft) handleBlockInfo(block *common.Block) {
	...
	if err != nil {
		m.log.Errorf("start HighWay failed, error: %+v", err)
		return
	}
	...
	if currentMode == protocol.CONSENSUS_VERIFY {
		m.msgBus.Register(msgbus.RecvConsensusMsg, m)
		m.epoch.Start()
	}
}
...
func (m *Maxbft) start() {
	go func() {
		m.log.Infof("start HighWay engine, first receive block by sync module")
...
func (m *Maxbft) stop() error {
	m.msgBus.UnRegister(msgbus.RecvConsensusMsg, m)
	m.epoch.Stop()
	return nil
}
